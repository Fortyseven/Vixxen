...
var labelManager = {
    ...
    enable(savestate = true) {
        for (var i in this._labels) {
            this._labels[i].timer = setInterval(function () {
                this._processLabel(this._labels[i].entry);
            }.bind(this), this._labels[i].entry.interval);
        }
...
export default {
    install(Vue, options) {
        ...
        labelManager._is_running = labelManager._vue.$store.getters.getLabelState;
